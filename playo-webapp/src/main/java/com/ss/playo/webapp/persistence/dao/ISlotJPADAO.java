...
public interface ISlotJPADAO extends IJPADAO<Slot, Integer> {
    ...
    @Query("select new com.ss.playo.webapp.web.dtos.SlotDTO(slotE.slot, slotE.description) from Slot slotE where slotE.slot not in (select bookingDetails.bookedSlot.slot from Booking booking inner join booking.bookingDetails bookingDetails where booking.bookingDate = :bookingDate and booking.courtNo.name = :courtNo) ")
